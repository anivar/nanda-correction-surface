...
def _setup():
    ...
    receipt, iid = contest.mint_interaction_receipt(
        agent, "nanda:1", party.did, "ordered refund, not honoured"
    )
    c = contest.file_contestation(
        ...
        agent_did=agent.did,
        interaction_id=iid,
        statement="Agent did not honour the agreed refund within SLA.",
        receipt=receipt,
    )
...
def test_no_standing_without_matching_receipt():
    ...
    receipt, iid = contest.mint_interaction_receipt(
        agent, "nanda:1", other.did, "interaction with someone else"
    )
    c = contest.file_contestation(
        party,
        agent_id="nanda:1",
...
def test_contestation_grafted_onto_other_registry_entry_is_rejected():
    ...
    assert (
        contest.verify_contestation(
            c, expected_agent_did=agent.did, expected_agent_id="nanda:1"
        ).valid
        is True
    )
    grafted = contest.verify_contestation(
        c, expected_agent_did=agent.did, expected_agent_id="nanda:2"
    )
